...
void systemStartup()
{
  ...
  while (millis() - startTime < 2000)
  {
    ...
    if (twai_receive(&rx, pdMS_TO_TICKS(10)) == ESP_OK)
    {
          ...
          rx.data[1] == PARAM_SYSTEM_BOARD_COMPLETE)
      {
        ...
        else if (boardType == BOARD_TYPE_ROUTING)
        {
          routingReady = true;
          Serial.println("Routing board ready");
        }
      }
    }
  }
  ...
  routingBoardDetected = routingReady;
  if (routingReady)
    Serial.println("Routing active");
  else
    Serial.println("WARNING: Routing board not detected — no routes will be applied");
...
//
// IC3 bits (oscillator boards):
//   bit 2 = OSC_A fold enabled   bit 3 = OSC_A S&H enabled
//   bit 9 = OSC_B fold enabled   bit 8 = OSC_B S&H enabled
//
// IC4 bits (oscillator boards):
//   bit 0 = CLONE_A  bit 1 = SOLO_A  bit 2 = FM_ENABLED_A
//   bit 3 = SOLO_B   bit 4 = CLONE_B bit 5 = SYNC_B
//
// CLONE bits: driven by global cloneActive flag.
//   First clone press: cloneActive=true  → all clone LEDs on all boards lit.
//   Second clone press: cloneActive=false → all clone LEDs off.
//   Cloned parameters remain on all voices regardless of LED state.
static void computeOscBoardLeds(int boardIdx, uint16_t &ic3, uint8_t &ic4)
{
  ic3 = 0;
  ic4 = 0;
  ...
  if (vA < MAX_VOICES)
  {
    ...
    if (cloneActive)            ic4 |= (1 << 0);  // CLONE_A — all boards when clone active
    if (voices[vA].soloEnabled) ic4 |= (1 << 1);
    if (voices[vA].fmEnabled)   ic4 |= (1 << 2);
  }
  if (vB < MAX_VOICES)
  {
    ...
    if (cloneActive)            ic4 |= (1 << 4);  // CLONE_B — all boards when clone active
...
void sendCanAck(uint8_t target, uint8_t param, uint8_t /*seq*/)
{
  ...
  {
    int boardIdx = (target - TARGET_OSC1) / 2;
    computeOscBoardLeds(boardIdx, ic3, ic4);
  }
  else
  {
    // Non-OSC targets — ACK goes to commons board.
    // Send current commons IC3 state so it can maintain its remote LEDs.
  ...
  while (twai_receive(&msg, 0) == ESP_OK)
  {
    ...
    if (v >= 0 && v < NUM_VOICES_ACTIVE)
    {
      switch (param)
      {
        ...
        case PARAM_OSC_SOLO:
        {
          // When FM is enabled on this board pair, treat it as a single tonal voice.
          // Either solo button solos the whole pair — otherwise only the carrier is
          // heard without its modulator and FM disappears entirely.
          // Always toggle using the even (master) voice as truth, then mirror to odd.
          int vEven = (v / 2) * 2;
          int vOdd  = vEven + 1;
          if (voices[vEven].fmEnabled && vOdd < NUM_VOICES_ACTIVE)
          {
            voices[vEven].soloEnabled = !voices[vEven].soloEnabled;
            voices[vOdd].soloEnabled  =  voices[vEven].soloEnabled;
          }
          else
          {
        ...
        case PARAM_OSC_CLONE:
          // Clone all timbre parameters from src voice to all other voices.
          cloneVoice((uint8_t)constrain(value, 0, NUM_VOICES_ACTIVE - 1));
          // Toggle clone LED state on all boards.
          // First press: cloneActive=true  → all clone LEDs light on all boards.
          // Second press: cloneActive=false → all clone LEDs extinguish.
          // Cloned parameters remain on all voices regardless of LED state.
          cloneActive = !cloneActive;
          // Broadcast updated ACK to every oscillator board so all clone LEDs update.
          for (int b = 0; b < (NUM_VOICES_ACTIVE / 2); b++)
          {
            int tgt = TARGET_OSC1 + (b * 2);  // OSC_A of each board
            sendCanAck((uint8_t)tgt, param, seq);
          }
          // Skip the standard sendCanAck at end of loop for this message
          // since we've already sent to all boards above.
          continue;
    ...
    else if (target == TARGET_ROUTE)
    {
      switch (param)
      {
        ...
        {
          // value high byte = route index (0-11)
          uint8_t routeIndex = (uint8_t)(value >> 8);
          if (routeIndex < MAX_ROUTES)
            routes[routeIndex].source = value & 0xFF;
          break;
        }
        case PARAM_ROUTE_OSC_MASK:
        {
          uint8_t routeIndex = (uint8_t)(value >> 8);
          if (routeIndex < MAX_ROUTES)
            routes[routeIndex].oscMask = value & 0xFF;
          break;
        }
        case PARAM_ROUTE_PARAM_MASK:
        {
          uint8_t routeIndex = (uint8_t)(value >> 8);
          if (routeIndex < MAX_ROUTES)
            routes[routeIndex].oscParamMask = value & 0xFF;
          break;
        }
        case PARAM_ROUTE_GLOBAL_MASK:
        {
          uint8_t routeIndex = (uint8_t)(value >> 8);
          if (routeIndex < MAX_ROUTES)
            routes[routeIndex].globalMask = value & 0xFF;
          break;
        }
        case PARAM_ROUTE_AMOUNT:
        {
          ...
          if (routeIndex < MAX_ROUTES)
          {
            // value low byte = amount 0-198, centre=99 → normalised -1.0 to +1.0
            int8_t amt = (int8_t)((value & 0xFF) - 99);
            routes[routeIndex].amount = amt / 99.0f;
          }
          break;
        }
        ...
        case PARAM_ROUTE_SAVE:
        {
          uint8_t routeIndex = (uint8_t)constrain(value, 0, MAX_ROUTES - 1);
          routes[routeIndex].enabled = true;
          break;
        }
        case PARAM_ROUTE_CLEAR:
        {
          uint8_t routeIndex = (uint8_t)constrain(value, 0, MAX_ROUTES - 1);
          routes[routeIndex] = Route();  // reset to defaults
          break;
        }
