...
void noteOn(uint8_t note, uint8_t velocity)
{
  ...
  if (playMode == MODE_POLY)
  {
    ...
    // If this is an FM master, start its slave on the same note.
    // The slave's pitch must match the master's so the FM modulation index
    // is consistent — a mismatched slave frequency produces random-sounding
    // sidebands rather than a stable FM timbre.
    if (v % 2 == 0 && v + 1 < NUM_VOICES_ACTIVE && voices[v].fmEnabled)
    {
      int vB = v + 1;
      voices[vB].note          = note;
      voices[vB].phase         = (uint32_t)(voices[vB].phaseOffset * 4294967296.0f);
      voices[vB].phaseInc      = voicePhaseInc(freq, voices[vB].octave, voices[vB].detuneCents);
      voices[vB].active        = true;
      voices[vB].envState      = ENV_ATTACK;
      voices[vB].velocity      = velNorm;
      voices[vB].shPhase       = 0;
      voices[vB].prevPhaseNorm = 0.0f;
      voices[vB].chaosX        = chaosSeedFromNote(note);
      voices[vB].rootFreq      = freq;
      voices[vB].fltLow        = 0.0f;
      voices[vB].fltBand       = 0.0f;
      updateFilterCoeffs(voices[vB]);
    }
  }
...
void noteOff(uint8_t note)
{
  ...
  for (int i = 0; i < NUM_VOICES_ACTIVE; i++)
  {
    if (voices[i].note == note && voices[i].active)
    {
      ...
      // If this is an FM master, release its slave simultaneously.
      // The slave was started with the master in noteOn and must end with it.
      if (i % 2 == 0 && i + 1 < NUM_VOICES_ACTIVE && voices[i].fmEnabled)
        voices[i + 1].envState = ENV_RELEASE;
    }
  }
