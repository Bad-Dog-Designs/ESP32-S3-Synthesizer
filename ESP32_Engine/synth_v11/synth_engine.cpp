...
bool         routingBoardDetected = false;
...
// Clone Voice
// ==========================
// Copies all timbre parameters from the source board pair to all other board pairs.
// syncEnabled NOT cloned — sync is per-voice hardware button.
//
// FM PAIR MODE (fmEnabled on source board):
//   OSC_A and OSC_B have distinct roles (modulator/carrier, different waveforms,
//   fmDepth vs fmRatio). The entire board pair is treated as one FM voice.
//   Even voices receive the source OSC_A parameters.
//   Odd  voices receive the source OSC_B parameters.
//   Pressing either clone button on a board triggers this — src is normalised to
//   the board pair, so clone on OSC_B (src odd) still clones the full pair.
//
// NORMAL MODE (FM not enabled on source board):
//   All parameters copied from src to every other individual voice.
//   fmDepth and fmRatio ARE included so tonal character transfers completely.
void cloneVoice(uint8_t src)
{
  ...
  // Identify the source board pair regardless of which OSC button triggered clone.
  int srcEven = (src / 2) * 2;      // OSC_A of the source board
  int srcOdd  = srcEven + 1;        // OSC_B of the source board
  bool fmPair = (srcEven < NUM_VOICES_ACTIVE) && voices[srcEven].fmEnabled;

  for (int i = 0; i < NUM_VOICES_ACTIVE; i++)
  {
    if (fmPair)
    {
      // Skip both voices of the source board pair
      if ((i / 2) * 2 == srcEven) continue;

      // Even destination voices receive OSC_A (modulator) parameters.
      // Odd  destination voices receive OSC_B (carrier) parameters.
      int cloneSrc = (i % 2 == 0) ? srcEven : srcOdd;

      voices[i].oscType     = voices[cloneSrc].oscType;
      voices[i].pulseWidth  = voices[cloneSrc].pulseWidth;
      voices[i].chaosR      = voices[cloneSrc].chaosR;
      voices[i].foldEnabled = voices[cloneSrc].foldEnabled;
      voices[i].foldAmount  = voices[cloneSrc].foldAmount;
      voices[i].foldBias    = voices[cloneSrc].foldBias;
      voices[i].shEnabled   = voices[cloneSrc].shEnabled;
      voices[i].shRate      = voices[cloneSrc].shRate;
      voices[i].shRange     = voices[cloneSrc].shRange;
      voices[i].octave      = voices[cloneSrc].octave;
      voices[i].detuneCents = voices[cloneSrc].detuneCents;
      voices[i].phaseOffset = voices[cloneSrc].phaseOffset;
      voices[i].level       = voices[cloneSrc].level;
      voices[i].fmDepth     = voices[cloneSrc].fmDepth;
      voices[i].fmRatio     = voices[cloneSrc].fmRatio;
      voices[i].fmEnabled   = voices[cloneSrc].fmEnabled;
    }
    else
    {
      // Normal mode: copy src to every other individual voice.
      if (i == src) continue;
      ...
      voices[i].fmDepth     = voices[src].fmDepth;
      voices[i].fmRatio     = voices[src].fmRatio;
      voices[i].fmEnabled   = voices[src].fmEnabled;
    }
  }
...
// FM Slave Check
// ==========================
// Returns true if voice v is an FM-bound slave: an odd voice whose even partner
// has fmEnabled. FM slaves are not independently allocatable — they are always
// started and stopped together with their master by noteOn/noteOff.
static bool isFMSlave(int v)
{
  if (v % 2 == 0) return false;
  return (v - 1 >= 0) && voices[v - 1].fmEnabled;
}
...
int allocateVoice()
{
  // Tier 1: idle non-slave voice
  for (int i = 0; i < NUM_VOICES_ACTIVE; i++)
  {
    if (voices[i].active) continue;
    if (isFMSlave(i))     continue;  // slave is allocated with its master
    return i;
  }

  // Tier 2: releasing voice — skip attack voices and FM slaves
  int steal = -1;
  for (int i = 0; i < NUM_VOICES_ACTIVE; i++)
  {
    if (isFMSlave(i))                    continue;
  ...
  // Tier 3: all in attack — steal quietest non-slave voice
  steal = 0;  // voice 0 is always even, never a slave
